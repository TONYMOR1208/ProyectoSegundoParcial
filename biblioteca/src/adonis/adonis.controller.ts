...
@Controller('adonis')
export class AdonisController {
    constructor(private readonly adonisService: AdonisService) {}

    @Get(':route')
    async getDataFromRoute(@Param('route') route: string, @Headers('Authorization') authHeader: string) {
        try {
            const token = authHeader ? authHeader.split(' ')[1] : null;
            const data = await this.adonisService.getDataFromRoute(route, undefined, token);
            return data;
        } catch (error) {
    ...
    async getDataFromRouteWithId(@Param('route') route: string, @Param('id') id: string, @Headers('Authorization') authHeader: string) {
        try {
            const token = authHeader ? authHeader.split(' ')[1] : null;
            const data = await this.adonisService.getDataFromRoute(route, id, token);
            return data;
        } catch (error) {
            if (error instanceof HttpException) {
                throw error;
    ...
    async getDefaultRoute(@Headers('Authorization') authHeader: string) {
        try {
            const token = authHeader ? authHeader.split(' ')[1] : null;
            const data = await this.adonisService.getDataFromRoute(undefined, undefined, token);
            return data;
        } catch (error) {
            if (error instanceof HttpException) {
                throw error;
    ...
    async postDataToRoute(@Param('route') route: string, @Body() body: any, @Headers('Authorization') authHeader: string) {
        try {
            const token = authHeader ? authHeader.split(' ')[1] : null;
            const response = await this.adonisService.postDataToRoute(route, body, token);
            return response;
        } catch (error) {
            if (error instanceof HttpException) {
                throw error;
